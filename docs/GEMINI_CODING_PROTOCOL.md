...
- 直接运行会写数据的测试并连接默认 `localhost:3000`。
- 把页面空状态直接判断为数据库记录已丢失。
- 只检查 `projects` 而漏查 `project_files`、`project_figures`、`sessions`、`export_assets` 和物理文件。
...
涉及保存、历史、账号归属、文件或导出资产时，Preflight 还必须记录：

```text
npm run data:audit 的修改前结果
真实 data/scifigure.db 的大小和修改时间
本次页面/API 是账号级还是当前项目级
测试是否使用隔离 SCIFIGURE_DATA_DIR
是否存在可恢复数据库备份
```
...
- 主文档、专项方案、错误记录、部署说明和测试报告每次修改时，都更新文件头部的“最后修改时间”或“更新时间”。
- 错误日志中的单条记录必须使用实际发生或修复时间，不能只写模糊日期。
- 修改与历史问题相关的模块前，必须先搜索 `docs/ERROR_LOG.md`，确认是否存在同类错误和既有回归约束。
- 已修复错误不能从日志中删除；若原结论变化，应追加“复核/修正”记录并写明新时间。
- 修复完成后必须把防复发规则落实为测试、类型约束、状态门禁或明确的验收步骤，不能只记录文字结论。
- 文档中不得把“计划”“代码已写”“自动测试通过”“真实浏览器验证”和“生产验证”混为同一完成状态。
