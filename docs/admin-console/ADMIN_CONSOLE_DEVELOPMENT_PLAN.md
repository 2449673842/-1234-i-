...
> 实现校准：Phase A、主要 Phase B、Phase C4 与管理员 MFA 代码已完成；公网部署、最后管理员保护和其余受控操作仍待完成
> 模块定位：独立安全运维模块  
> 与绘图主线关系：复用认证和数据库，不依赖编辑器、Figure 协议或渲染 UI  
> 默认开关：关闭  
> 开发原则：先只读、后写入；先元数据、后受控操作；不提供任意命令、SQL 或用户文件浏览

> 当前验证：管理员后台专项已覆盖项目/Figure 归属、错误 DTO 去邮箱/指纹、其他用户原始身份最小化、metadata allowlist、AI handoff 脱敏、订阅 MFA step-up、MFA 登录/恢复码/防重放、幂等和角色即时撤销；最新本地安全改动尚待发布

> 最新证据：2026-07-16 01:55:49 +08:00，Vitest 42/42 文件、266/266 测试及管理员 MFA、后台只读、授权、部署生命周期、生产构建和 production bundle 均通过；全部使用临时数据与随机端口。
...
2FA 代码、离线引导和隔离测试已完成；真实 keyring、TLS、域名和服务器会话验证尚未完成。VPN、WAF 和云告警仍依赖部署环境，不能伪造“已完成”状态。
