...
Nginx 覆盖客户端传入的 `X-Forwarded-For`。TLS 配置使用固定域名跳转，证书续期后通过 deploy hook 执行 `nginx -t` 和 reload。模板同时配置 64 MB 请求体上限、慢请求超时、每 IP 请求速率和并发连接保护；Node 内单文件仍以 50 MB 为硬上限。
