...
当前五个中心已经具备各自的主要能力，但共享属性仍存在控件、别名、能力判断和批量范围不一致。专项方案 `docs/platform-capability/UNIFIED_EDITING_CENTERS_UPGRADE_PLAN.md` 规定后续采用“统一属性描述器 + renderer 属性能力 + 中心职责投影”的方式渐进迁移。

该规划不要求五个中心变成相同面板：属性编辑仍是单对象完整检查器，字体中心负责语义角色排版，组件中心负责结构类别，配色中心负责颜色绑定，布局中心负责几何。只有重叠属性的控件、目标解析、Draft、patch 和持久化语义必须一致。本文记录的是规划入口，不表示迁移已经完成。
