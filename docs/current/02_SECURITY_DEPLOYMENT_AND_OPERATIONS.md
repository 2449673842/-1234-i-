...
> 状态：当前有效  
> 更新时间：2026-07-14 15:10:52 +08:00
...
当前管理员授权/撤销仍通过服务器命令完成。2026-07-14 发布候选已将管理员后台 Phase A、部分 Phase B 和订阅修正切片合入统一编辑发布分支。生产环境仍必须显式设置 `SCIFIGURE_ADMIN_CONSOLE_ENABLED=1`；功能开关缺失时 `/admin` 和新增管理 API 保持关闭。服务器部署状态以发布后的 build ID 和线上验收记录为准。
