...
> 状态：当前有效  
> 更新时间：2026-07-15 22:17:55 +08:00
...
邮箱注册接口同时使用 `IP + email` 和单 IP 总量限制。为了避免账号枚举，已注册邮箱也会经过相同的邮件提供器调用，但验证阶段只返回“请直接登录”，绝不修改既有账号密码。生产邮件服务商仍需设置独立发送额度、退信监控和滥用告警；当前调试服务器尚未配置真实邮件服务，因此本轮邮箱安全分支不得直接部署。
...
2026-07-15 22:17 +08:00 已完成的本地结果：`npm test` 为 40/40 文件、244/244 测试；邮箱验证 smoke 覆盖验证前不创建用户、已注册/新邮箱同提供器路径、发送失败保留旧码、数据库唯一索引、密码策略前置和 HMAC-only 存储；公开注册、refresh、管理员只读/授权、安全基线、用户隔离、数据完整性工具、仓库边界、生产 bundle 和生产构建均通过。所有 API/浏览器测试使用系统临时目录和随机端口，没有连接本地 3000、真实数据库或真实项目目录。
...
登录失败计数与账号级持久冷却尚未实现；当前主要依赖进程内 IP/标识符限流
