...
邮箱发送另有纯收件地址 HMAC 预算，攻击者更换 IP 也不能持续轰炸同一邮箱
验证码由随机 challenge ID 与强密钥确定性派生，数据库仍只保存 code HMAC
邮件发送前先持久化不可验证候选和 outbox 租约，provider 接受后原子激活并替换旧 challenge
进程在发送前、中、后退出时，启动 worker 会重发同一 challenge/验证码；明确发送失败保留旧 challenge
同邮箱并发注册收敛到同一在途 challenge 和一次 provider 调用
...
npm run test:email-outbox-recovery
...
网络结果未知或激活事务中断时，outbox 可能向同一邮箱重复发送同一个验证码；真实邮件提供器的幂等能力、送达/退信与重复率监控仍待域名配置后验证
```
