...
> 状态：当前有效  
> 更新时间：2026-07-15 22:45:42 +08:00
...
2026-07-15 22:45 +08:00 已完成的本地结果：`npm test` 为 41/41 文件、248/248 测试；邮箱验证 smoke 覆盖验证前不创建用户、已注册/新邮箱同提供器路径、发送失败保留旧码、数据库唯一索引、密码策略前置和 HMAC-only 存储；认证 throttle smoke 覆盖存在/不存在账号同响应与 dummy 工作、失败计数跨服务重启、Retry-After 冷却、陌生设备阻断、可信设备正确密码软解锁、PBKDF2 迁移和 HMAC-only 计数存储。公开注册、refresh、管理员只读/授权、安全基线、用户隔离、数据完整性工具、仓库边界、生产 bundle 和生产构建均通过。所有 API/浏览器测试使用系统临时目录和随机端口，没有连接本地 3000、真实数据库或真实项目目录。
