...
> 状态：历史对比有效，已补充 2026-07-16 网页更新候选
> 创建时间：2026-07-15 11:34:43 +08:00
> 最后更新：2026-07-16 20:41:53 +08:00
> 本地对象：`http://127.0.0.1:3000`，根工作区 `feature/standard-figure-model-v1`
> 网页对象：`http://117.72.208.91`，线上 build `864ce62-jd19`
> 说明：文中“3200 版本”指统一编辑中心升级分支。3200 是早期隔离验证端口，不是当前线上长期端口。

## 0. 当前状态校准

本文第 2-7 节的提交数量、文件差异和待发布项保留为 `d625000-jd14` 时点的历史对比证据，不应解释为 `864ce62-jd19` 的实时统计。

截至 2026-07-15 21:14:30 +08:00：

- 本地 `3000` 与网页统一版均修复全局 `/api/export-assets` 未进入认证请求包装器的问题，真实浏览器确认资产库请求携带 Bearer Token。
- 网页当前不可变 release 为 `864ce62-jd19`，live/ready 正常且只运行一个 SciFigure 实例。
- 重复发布的 npm、renderer Debian、Debian Security 和 PyPI 默认使用已验证国内镜像；Ubuntu、Docker CE 和 rootless Docker 加速配置已纳入部署基线。
- NodeSource 签名仓库和微软 Core Fonts 暂保留上游来源，不使用来源不明或授权边界不清晰的替代镜像。
- 2026-07-16 网页更新候选已在 worktree 完成，但尚未替换线上 `864ce62-jd19`。下文新增候选能力不得当作线上现状。
...
### 3.9 2026-07-16 网页更新候选

本轮在保留网页严格 resolver 和生产边界的前提下，同步本地近期可用交互并新增导出状态恢复：

- 单击图元后，组件、字体和布局中心自动跟随其所属子图；配色中心投影当前选中目标，跨子图多选回退到全部子图。
- V2 数值控件有效输入立即暂存；增加字体格式刷、图例符号/文字间距和边距、散点相对比例缩放、保持尺寸的垂直行间距。
- Ctrl/Command 可从任意数量的右侧列表选择中取消单个对象；框选后下一次真实单击不再被残留抑制标记吞掉。
- 向量 collection 的多个颜色子集使用 `matchColor` 独立暂存和重放，避免改蓝组时覆盖红组。
- 每个新导出资产原子绑定不可变编辑快照，资产库可恢复到导出时状态；恢复前保存当前检查点并校验项目、脚本和数据。
- 导出、恢复、上传和删除进入同一项目级独占边界；导出期间上传/删除返回项目忙。“保存全部 Figure”会检查所有 Figure 的未应用 Draft。

该候选目前只完成本地代码、真实浏览器/API 回归和生产构建，尚未发布到公网。
...
| 数值暂存/格式刷 | 部分专用控件已同步 | V2 有效输入立即暂存 + 字体格式刷 | 不再普遍依赖 Enter，格式只复制字体样式 |
| 子图作用域跟随 | 各中心需重复选择 | 选择图元后跨中心自动跟随 | 减少重复选择，跨子图多选安全回退 |
...
| 导出时状态恢复 | 依赖撤销/历史手动回退 | 资产级不可变快照 + 恢复前检查点 | 可直接返回某次导出时的脚本、Figure editLog 和 revision |
...
2026-07-16 候选进一步禁止在项目任一 Figure 仍有 Draft 时“保存全部 Figure 到图库”，并把每次新导出绑定到可校验、可恢复的编辑快照。用户后续继续修改后，可以从资产库恢复到该次导出状态；恢复前的更新状态仍保存在历史检查点中。

### 5.4 可部署和可恢复

网页版本把开发应用变成了可发布服务：候选版本在后台构建，只有 renderer、前端、readiness 全通过后才切换；失败自动恢复上一 release。用户数据库不进入发布包，发布前后检查 SQLite 完整性和计数。

导出快照恢复使用单 Node 进程内项目锁，和当前单 SQLite writer 部署一致。未来改为多进程/多节点前，必须先引入数据库或外部协调锁，不能直接水平扩容写服务。
...
- 2026-07-16 候选：TypeScript、41 个 Vitest 文件/250 项、Python 48 项、R 30 项和生产构建通过。
- Python 语义 16/16、组件/布局 24/24、严格行为 15/15、扩展拖拽、R 语义 7/7、作用域跟随和导出快照 DB/API/UI 通过。
- 导出快照 API 实测同名额外数据拒绝、恢复前检查点、在途任务 draining、导出期间上传/删除阻断、所有权和旧资产兼容。
