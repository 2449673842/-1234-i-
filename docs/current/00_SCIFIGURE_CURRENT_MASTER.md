...
2026-07-15 22:17 +08:00 本轮邮箱注册与管理员隐私收尾的本地证据：40 个 Vitest 文件、244 个测试全部通过；邮箱验证、公开注册、refresh、管理员只读/授权、安全基线、用户隔离、数据完整性工具、生产 bundle、仓库边界和生产构建通过。验证覆盖账号状态同路径、发送失败保留旧码、有效挑战数据库唯一性、验证前无永久用户/密码存储以及管理员运营文本脱敏。测试全部使用临时数据和随机端口，没有修改本地 3000、Docker 或真实用户数据。本轮安全分支尚未部署，真实邮件提供器、域名和 TLS 仍是上线阻断项。
