...
当前公网调试服务器基线为不可变 release `864ce62-jd19`。本地 `3000` 与网页统一版的全局导出资产库认证请求均已通过真实浏览器回归；重复发布默认使用受权限保护的国内优先镜像配置。
...
| 能力 | 当前状态 | 说明 |
|---|---|---|
| 公开宣传页与注册门禁 | 本地安全实现完成，邮箱生产通道待配置 | 验证成功前不创建永久用户或保存密码；新/旧邮箱走同一提供器路径，发送失败保留旧码；生产配置不完整时拒绝启动，当前尚未发布 |
...
| Draft Batch | 已实现 | 多项参数暂存后一次重绘，减少重复渲染 |
| 撤销、重做和保存 | 已实现 | 仍需持续验证刷新后持久化和历史锚点 |
...
| 管理员后台 | 旧版页面已部署；本地安全分支新增强制 MFA 与身份最小化 | 其他用户只显示脱敏账号标识，不返回原始邮箱或昵称；TOTP 登录、恢复码、短时重认证和密钥轮换已在隔离测试通过；公网版本仍未部署 |
...
验证前不创建用户或保存待注册密码，发送失败不破坏旧验证码
邮箱注册账号状态不可枚举、数据库有效挑战唯一性和统一密码策略
邮箱发送与登录尝试持久预算、跨重启账号失败冷却和可信设备软解锁
邮箱 challenge 先以不可验证候选和持久 outbox 入库，服务商接受后才原子激活
outbox 使用租约、确定性验证码和启动恢复，同邮箱并发请求收敛到一次发送
存在/不存在账号执行对齐的 PBKDF2 + Argon2 密码工作
user_id 所有权隔离
服务端随机 Figure session ID，冲突时拒绝写入且永不转移既有所有者
数据库 admin 角色和管理审计
管理员 TOTP 登录挑战、MFA 会话、一次性恢复码和订阅写入 step-up
管理员 TOTP seed 使用带 key ID 的 AES-256-GCM 密文保存，恢复码只保存高熵单向摘要
管理员列表和订阅接口不返回其他用户原始邮箱或昵称
API、认证、渲染和管理接口限流
...
脚本、内联数据、编辑记录、Figure 历史、临时会话和项目目录均有独立安全容量边界
multipart 上传在 Multer 读取前完成可计量长度与小时预算门禁，组合项目复制文件复用同一存储预算
持久渲染缓存、幂等响应缓存和导出资产经过数量、字节、SVG、元数据和公开路径约束
管理员错误中心项目归属验证和内容最小化
...
2026-07-16 01:55:49 +08:00 本轮邮箱注册、管理员隐私、MFA、认证防滥用和持久化资源边界的本地证据：42 个 Vitest 文件、266 个测试全部通过；管理员 MFA、后台只读/授权、邮箱验证、refresh、部署生命周期、生产 bundle、仓库边界和生产构建通过。Conda 环境内真实 R renderer 29 项、Python/R capability matrix 2 项通过，测试通过 `RSCRIPT_BIN` 显式选择环境内 R。验证覆盖验证前无永久用户/密码存储、管理员登录 challenge、设备绑定、TOTP/恢复码防重放、角色变更注销会话、原始身份不出后台、provider/outbox 故障恢复以及既有 session/上传/缓存/导出边界。测试全部使用临时数据和随机端口，没有修改本地 3000、Docker 或真实用户数据。本轮安全分支尚未部署，真实邮件提供器、域名、TLS 和同 commit Linux renderer 镜像复测仍是上线阻断项。
...
`2026-07-12 14:57:42 +08:00` 恢复历史导出资产：目标真实账号（文档已脱敏）的 81 条导出记录全部确认归属正确，78 个原文件仍在，3 个缺失文件由数据库保存的 SVG 图形内容重建，最终达到 81/81 文件存在。资产库改为默认汇总当前账号全部项目，并支持按来源项目筛选。
