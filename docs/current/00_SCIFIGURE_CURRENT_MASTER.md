...
| 管理员后台 | Phase A、部分 Phase B、订阅修正切片已部署；本地隐私边界已加强 | 后台只读取账号/订阅和脱敏运行元数据；订阅备注与审计 metadata 双重脱敏，不读取用户脚本、数据、SVG、Figure、导出内容、路径或秘密；最新本地改动尚待发布 |
...
2026-07-16 00:27 +08:00 本轮邮箱注册、管理员隐私、认证防滥用和持久化资源边界的本地证据：41 个 Vitest 文件、256 个测试全部通过；邮箱验证、邮箱 outbox 崩溃恢复、认证 throttle、refresh、管理员只读/授权、安全基线、双用户隔离、缓存、组合代码项目、导出矩阵、R 风险预检、部署生命周期、生产 bundle、仓库边界和生产构建通过。验证覆盖验证前无永久用户/密码存储、provider 接受前候选已持久化但不可验证、接受后原子激活、进程崩溃后重发同一 challenge/验证码、发送失败保留旧 challenge、同邮箱并发只调用一次 provider、outbox 无明文邮箱/验证码、旧数据库 challenge 无损迁移、最终发送租约不被其他 worker 提前废弃，以及前一批 session/上传/缓存/导出边界。测试全部使用临时数据和随机端口，没有修改本地 3000、Docker 或真实用户数据。本轮安全分支尚未部署，真实邮件提供器、域名、TLS 和同 commit Linux renderer 镜像复测仍是上线阻断项。
