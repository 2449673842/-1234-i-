...
2026-07-16 的上传/导入安全加固进一步退役旧 `DataImportPage`，将历史 `data_import` 状态迁移到统一新建项目页。CSV 浏览器分析固定为最多 100 行；`.xlsx` 只有通过中央目录、本地文件头、data descriptor、路径、条目数、展开量和压缩比校验且处于浏览器预算内才预览，旧 `.xls` 一律交给服务端隔离解析。项目创建后使用服务端权威列名、行数和 100 行预览确定最终字段映射，浏览器样本不再写入 `spec.raw_data.custom_data`。
...
浏览器 CSV 100 行样本、工作簿单队列解析、XLSX 本地/中央目录一致性校验和旧 XLS 服务端专用解析
50 MB 单文件硬上限、项目/账号/全平台存储上限、上传下载频率和原子化持久小时字节预算
项目文件与导出资产的所有权、累计存储检查和数据库登记使用 SQLite IMMEDIATE 事务，跨 Node 进程不能超额
脚本、内联数据、编辑记录、Figure 历史、临时会话和项目目录均有独立安全容量边界
multipart 上传在 Multer 读取前完成可计量长度与小时预算门禁，组合项目复制文件复用同一存储预算
组合项目不信任历史文件 metadata，复制前重新校验源字节、列结构和行数，失败只回滚新目标项目
...
2026-07-16 03:14:56 +08:00 本轮邮箱注册、管理员隐私、MFA、认证防滥用、上传解析和持久化资源边界的本地证据：43 个 Vitest 文件、273 个测试全部通过；管理员 MFA、后台只读/授权、邮箱验证与 outbox 恢复、refresh、用户隔离、认证限流、Free/Pro `observe`、安全基线、跨进程上传/导出配额、真实项目创建、组合项目、导航重新配置、广泛浏览器行为、SVG 净化、项目历史、部署生命周期、生产 bundle、部署包、仓库边界和生产构建通过。Conda 环境内真实 R renderer 29 项、Python/R capability matrix 2 项、表格 parser 5 项和 R 安全预检通过，测试通过 `RSCRIPT_BIN` 显式选择环境内 R。验证覆盖浏览器单队列解析、XLSX 正常/恶意结构、旧 XLS 延迟解析、服务端权威字段映射、项目 spec 不保存完整表格、跨进程配额不超额和失败无文件/数据库残留。全部 API/浏览器测试使用临时数据和随机端口，没有修改本地 3000、Docker 或真实用户数据。本轮安全分支尚未部署，真实邮件提供器、域名、TLS、备份恢复和同 commit Linux renderer 镜像复测仍是上线阻断项。
