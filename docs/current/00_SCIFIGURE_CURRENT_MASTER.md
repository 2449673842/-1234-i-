...
> 状态：当前有效  
> 更新时间：2026-07-15 11:45:53 +08:00
...
最新本地 3000 与网页统一编辑版差异见 `04_LOCAL_3000_VS_WEB_UNIFIED_COMPARISON_2026-07-15.md`。该文件已于 2026-07-15 11:45:53 +08:00 从统一编辑工作树同步到根项目，便于直接查看。
...
P2：渲染速度、队列、缓存和大文件性能
P3：管理员后台、更多编辑能力和 AI 接入
```

服务器尚未购买，因此防火墙、TLS、加密云盘、SSH、真实备份和恢复演练仍属于待部署事项。相关代码和运行手册应先准备完成，部署后再做环境级验收。
...
| 能力 | 当前状态 | 说明 |
|---|---|---|
| 公开宣传页与注册门禁 | 已实现 | 匿名访问先进入宣传页；主视觉来自全合成 Figure；注册或登录后开放工作台；支持首屏纵深、滚动显现和安全生命周期动效，并尊重系统“减少动态效果”；安全文案隐藏内部防护细节 |
