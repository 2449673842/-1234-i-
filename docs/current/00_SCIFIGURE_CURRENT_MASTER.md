...
2026-07-15 23:43 +08:00 本轮邮箱注册、管理员隐私、认证防滥用和持久化资源边界的本地证据：41 个 Vitest 文件、254 个测试全部通过；邮箱验证、认证 throttle、refresh、管理员只读/授权、安全基线、双用户隔离、缓存、组合代码项目、导出矩阵、R 风险预检、部署生命周期、生产 bundle、仓库边界和生产构建通过。验证覆盖账号状态同路径、验证前无永久用户/密码存储、管理员运营文本脱敏、登录失败跨重启冷却、相同脚本跨用户 session 隔离、会话所有者不可转移、Figure 历史超限事务回滚、组合项目数据复制预算、上传前流量计量、缓存 SVG 安全规范化和导出资产公开路径最小化。测试全部使用临时数据和随机端口，没有修改本地 3000、Docker 或真实用户数据。本轮安全分支尚未部署，真实邮件提供器、域名、TLS 和同 commit Linux renderer 镜像复测仍是上线阻断项。
