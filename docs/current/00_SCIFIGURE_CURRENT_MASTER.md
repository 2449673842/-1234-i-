...
> 状态：当前有效  
> 更新时间：2026-07-30 15:51:34 +08:00
> 证据截止时间：2026-07-30 15:51:34 +08:00
> 复核范围：生产 release `e35f4a4-jd22` 与隔离集成分支 `deploy/prod-integration-v3`；含 Python WP3-WP10 capability 权威、组件能力/Draft 收敛、旧项目兼容、导出快照、统一能力报告、性能诊断和逐域回滚，R-WP0-WP3 隔离基线、patch 权威、结构 identity v2、旧项目兼容和运行时 parity，以及 R-WP4 Point/Jitter、Line/Path/Smooth、Bar/Col、Errorbar family、Boxplot/Violin、Ribbon/Area 本地候选；候选尚未部署
...
| R 渲染与语义编辑 | R-WP0-WP3 完成；R-WP4 前六类家族本地候选 | ggplot2 主链路可用；patch mode 服务端权威、失败零持久化、identity v2 和运行时 parity 已验证；Point/Jitter、Line/Path/Smooth、Bar/Col、Errorbar family、Boxplot/Violin、Ribbon/Area 已有专用 adapter。Ribbon/Area 保留旧 `r.layer.N`、`kind=patch` 和原 role，仅开放 body fill/outline/linewidth/alpha，区间、堆叠和 Smooth 置信带保持只读；整层 fill override 后从本次原始脚本 baseline 恢复 scale/guide 结构身份但不恢复旧样式，已休眠的 scale group 在 patch 与项目保存入口都明确拒绝。R remap 只允许当前 manifest 中实际存在的请求 GID 在同规范化家族内唯一迁移。离散分组配色已通过 Draft、保存刷新、撤销重做和导出 bundle。下一家族为 Step/Histogram/Freqpoly；候选尚未部署 |
...
`PYTHON_EDITING_COMPLETION_EXECUTION_PLAN.md` 与 `R_EDITING_COMPLETION_EXECUTION_PLAN.md` 分别收敛两套 renderer 的对象身份、patch 事务、复杂图元和用户链路。两份计划共用前端协议和不可回退基线，但不把 Matplotlib artist tree 与 ggplot/grob 强行合并。R-WP0-WP3 已完成本地候选；R-WP4 目前完成 Point/Jitter、Line/Path/Smooth、Bar/Col、Errorbar family、Boxplot/Violin 与 Ribbon/Area 本地候选，后续从 Step/Histogram/Freqpoly 继续逐个收敛，再进入网络图、路径图和 SEM 专用语义扩展。
