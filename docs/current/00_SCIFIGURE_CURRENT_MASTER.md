...
2026-07-15 22:45 +08:00 本轮邮箱注册、管理员隐私与认证防滥用的本地证据：41 个 Vitest 文件、248 个测试全部通过；邮箱验证、认证 throttle、公开注册、refresh、管理员只读/授权、安全基线、用户隔离、数据完整性工具、生产 bundle、仓库边界和生产构建通过。验证覆盖账号状态同路径、发送失败保留旧码、有效 challenge 数据库唯一性、验证前无永久用户/密码存储、管理员运营文本脱敏、登录失败跨重启冷却、可信设备软解锁、HMAC-only 计数存储以及旧 PBKDF2 登录迁移。测试全部使用临时数据和随机端口，没有修改本地 3000、Docker 或真实用户数据。本轮安全分支尚未部署，真实邮件提供器、域名和 TLS 仍是上线阻断项。
