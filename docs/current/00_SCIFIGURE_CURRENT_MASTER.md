...
2026-07-16 网页统一版更新候选已完成本地实现与回归，但尚未部署替换线上 `864ce62-jd19`。候选增加编辑中心自动跟随所选子图、导出资产编辑快照与一键恢复、项目级导出独占锁，以及本轮图例、散点、布局、框选、暂存和配色修复。线上能力必须以实际 release 标识为准。
...
| 编辑中心子图跟随 | 网页候选已实现，待发布 | 选择图元后组件、字体、布局和配色目标自动跟随其所属子图；跨子图多选安全回退到全部子图 |
...
| 导出状态快照与恢复 | 网页候选已实现，待发布 | 每个新导出资产原子绑定不可变编辑快照；恢复前校验账号、项目结构、脚本风险和数据集精确哈希，并先保存当前状态检查点 |
