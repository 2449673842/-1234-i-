...
> 最后修改时间：2026-07-16 03:14:56 +08:00

---

## 2026-07-16 03:14:56 +08:00 上传/导入整表物化、跨进程配额竞态与组合复制信任旧元数据

**现象**

- 新建项目页曾在浏览器完整解析 CSV/XLS/XLSX，并把完整表格保存在 React state；创建项目时还可能把浏览器解析的全部行写入 `spec.raw_data.custom_data`。
- 快速增删文件会重新解析整个文件列表；多个工作簿解析任务可能重叠，占用浏览器 CPU 和内存。
- 项目文件和导出资产先检查配额、后登记数据库；同一 SQLite 前面运行多个 Node 进程时，多个请求可能同时看到旧配额快照。
- 组合代码项目复制来源数据时信任数据库里的历史列名和行数；源文件被破坏或替换后仍可能复制错误内容。
- 超过浏览器预览预算的工作簿没有本地行样本，最终字段映射可能用空数据猜测。

**根因**

- 旧 `DataImportPage` 和 `ProjectCreatePage` 把“前端预览”与“正式导入”混成同一条整表物化链路。
- 浏览器只按扩展名选择解析器，没有在 SheetJS 前验证 ZIP 中央目录、本地文件头、data descriptor、展开量和压缩比。
- 存储预算检查与 `project_files/export_assets` 插入不在同一 `BEGIN IMMEDIATE` 事务中。
- 组合复制把数据库 metadata 当成当前文件事实，没有重新验证源字节。

**修复**

- 新增 `clientTabularPreview`：CSV/TSV/TXT 最多分析 100 行；预算内 `.xlsx` 必须先通过签名、中央目录、本地文件头、压缩方法、descriptor、内部路径、重复/重叠条目、ZIP64、条目数、展开量和压缩比检查。
- 旧 BIFF `.xls` 一律延迟到服务端隔离 parser；超出浏览器预算的合法 `.xlsx` 也不在浏览器展开。
- 同一文件复用解析 Promise，所有工作簿经过单一队列；快速选择多个工作簿时最大解析并发固定为 1。
- 项目 spec 不再保存浏览器样本；上传后使用服务端权威列名、总行数和 `limit=100` 预览确定最终 `x/y/group`。
- 退役旧 `DataImportPage`，历史 `data_import` 导航统一迁移到 `project_create`。
- 项目创建/更新提交的 inline `custom_data` 复用 renderer 表格安全门禁；数据文件名、列名和单元格在 AI 提示词中明确标记为不可信数据。
- 上传落盘名加入 UUID；项目文件和导出资产的所有权、累计配额与数据库登记放入 SQLite `IMMEDIATE` 事务，登记失败删除本轮新文件。
- 组合项目复制前重新校验源文件签名、列名和行数；失败时只回滚新目标项目和复制文件，不改写来源项目。

**验证**

- `npm test`：43/43 文件、273/273 测试通过；`npm run lint` 与生产构建通过。
- 浏览器安全导入 smoke：两个延迟 XLSX 每个只读取一次，最大并发 1；最终字段映射来自服务端预览，浏览器完整行未进入项目 spec。
- 真实项目创建 smoke：浏览器 100 行样本、服务端 150 行总数、上传/预览/更新/重载全部通过。
- 安全基线：正常 XLSX API 上传返回正确列名和 2 行；伪 XLSX、二进制 CSV、超宽表和非法 inline data 被拒绝；失败更新保留旧 spec/script。
- 两个 Node 进程并发测试：项目最多 10 个文件严格成立；导出资产只有预算内 1 个成功；拒绝请求没有数据库记录或物理文件残留。
- 组合项目 smoke：损坏源文件被拒绝，目标项目和目录回滚；正常 Python/R 组合继续通过。

**遗留风险**

- 预算内 `.xlsx` 浏览器预览仍依赖 SheetJS，不能作为服务器安全边界。
- 进程在文件写入与 SQLite 登记之间被强杀，仍可能留下未登记孤儿文件；生产需增加只读审计和保守清理流程。
- 同 commit Linux Docker parser、真实大工作簿和 BIFF `.xls` 仍需云端回归，本轮尚未部署。

**防复发规则**

- 前端预览永远只能保存有界样本，完整表格不得进入 React state、sessionStorage、项目 spec 或 AI 提示词。
- 配额判断和数据库登记必须处于同一跨进程可串行化事务；只做“检查后插入”不算安全。
- 复制历史文件必须重新验证当前字节，不得信任旧 metadata。
- 安全测试必须同时证明恶意输入被拒绝、正常 CSV/XLSX 可用、失败无残留和既有 Python/R 工作流未被误伤。

## 2026-07-16 03:14:56 +08:00 导出资产未登录异常被误包装为 500

**现象与根因**

- 进入导出资产库时，未携带或已失效 token 的请求显示 `500 Internal Server Error`，正文却是“未登录或登录已过期”。
- `authenticatedUserId()` 已给异常设置 `statusCode=401`，但项目列表和导出资产部分路由的局部 `catch` 写死 `res.status(500)`，丢失了认证语义。

**修复与验证**

- 项目列表/详情和导出资产列表、下载/删除相关 catch 统一保留 `err.statusCode`，真实内部错误仍返回 500。
- 导航重新配置测试补齐全局资产接口夹具，并记录所有 5xx 的方法、路径和响应摘要。
- 安全基线断言未认证 `/api/export-assets` 必须返回 401；导航/重新配置、资产返回和广泛浏览器行为回归通过。

**防复发规则**

- 认证/授权异常不得由局部 catch 强制改成 500；API smoke 必须同时检查状态码和统一错误结构。
- Playwright 遇到 5xx 必须记录具体 URL 和响应摘要，不能只保留浏览器“Failed to load resource”文本。
...
## 2026-07-16 00:27:33 +08:00 邮件已被 provider 接受但 challenge 尚未落库的崩溃窗口

**现象**

- 旧流程先调用邮件 provider，返回成功后才写入待注册 challenge。
- 若进程在两步之间退出，用户会收到六位验证码，但数据库不存在对应 challenge，验证码永远无法使用。
- 直接改成“先替换旧 challenge 再发送”又会在 provider 明确失败时破坏用户原有可用验证码。

**根因**

- 外部邮件发送是不可回滚副作用，旧代码却把唯一可恢复状态放在副作用之后提交。
- challenge 只有“未消费/已消费”两态，无法表达“已持久化但尚不可验证”的发送候选。
- 验证码由每次进程内随机生成，崩溃后无法在不保存明文的前提下重发同一码。

**修复**

- `pending_email_registrations` 增加 `deliverable_at`：空值为不可验证候选，非空值才进入同邮箱唯一可验证索引。
- 新增持久 `email_verification_outbox`，只保存 challenge 外键、状态、租约、尝试数和脱敏错误码，不保存收件地址、验证码或邮件正文。
- 六位码由随机 challenge UUID 与强验证密钥确定性派生，数据库继续只保存 code HMAC。
- provider 接受后，在同一 SQLite 事务中消费旧可验证 challenge、激活新 challenge 并确认 outbox。
- provider 明确拒绝时只废弃新候选；网络状态未知或进程中断时保留租约，由启动/定时 worker 重发同一 challenge 和验证码。
- 同邮箱并发请求复用同一在途 challenge；纯邮箱 HMAC 预算阻止攻击者换 IP 持续轰炸收件人。

**验证**

- 故障注入在本地 webhook 返回 200 后、激活前以退出码 86 终止隔离服务器。
- 重启后 worker 重发相同 delivery ID/验证码，候选从不可验证原子变为可验证并成功创建账号。
- provider 503 仍保留旧 challenge；并发两次注册只触发一次 provider 调用并返回同一 challenge。
- 旧 schema 有效 challenge 迁移后保持可验证，不生成伪造 outbox。
- 独立审查发现并修复“最终发送有效租约可能被另一 worker 提前废弃”的竞态；回归证明原租约仍可完成激活。
- TypeScript、41 个测试文件 256 项单测、邮箱/认证/管理员/安全基线、生产构建和 bundle 验证通过。

**防复发规则**

- 外部不可回滚副作用前必须先建立可恢复状态，但候选状态不得提前获得业务权限。
- provider 接受与数据库激活错误必须分开处理；接受后禁止按普通发送失败废弃 challenge。
- 多 worker 清理 `sending` 任务时必须尊重有效租约，最终尝试也不能例外。
- outbox 不得保存验证码、收件地址副本或邮件正文；恢复数据必须可由受保护的既有状态派生。
- 邮箱流程必须同时测试明确失败、网络未知、并发、进程崩溃、重启恢复和旧数据库迁移。
