...
- 状态：已随不可变 release `336f64e-jd30` 部署；本轮未触碰本地 `3000`，线上数据审计部署前后数量一致且为 0 issue。
...
- 生产部署前 SQLite 在线快照通过完整性检查；release 与 renderer image 均为 `336f64e-jd30`，systemd active、重启次数 0、公网/Node readiness ready、最近 warning 日志为空。
- 生产数据审计部署前后均保持 6 用户、52 项目、251 文件、74 导出资产和 0 issue；人工整版回退目标为 `e35f4a4-jd22`。
...
## 2026-07-29 18:07:45 +08:00 本地 R 运行时 smoke 将正常冷启动与最小超时阈值混为同一门禁

**状态与级别**

- 状态：已关闭。修正后的独立 smoke 通过，正常 R 冷启动、确定性警告、错误分类、结构化 timeout 和 job 清理均已复证；未改变产品默认 R 超时、renderer 行为或用户项目。
- 级别：P2 发布验收可靠性。普通 `ggplot2` 冷启动在本机 Windows 上可能略高于 5 秒，即使 renderer 正常，也会被用于验证 timeout 清理的同一 5 秒阈值误判为产品失败。
- 范围：`tests/api/r_runtime_consistency_smoke.mjs` 的隔离本地 R 测试；不影响 Docker renderer、3000、线上服务或真实数据。

**根因与修复**

- 该 smoke 用 `SCIFIGURE_R_TIMEOUT_MS=5000` 同时验证普通冷启动渲染、确定性警告、错误分类和超时清理；服务端也将 5 秒作为可配置的最低值。当前机器上的正常首个 `Rscript --vanilla`/`ggplot2` SVG 渲染稳定触及该极限。
- 测试现在给正常运行时一致性渲染 15 秒预算，并把故意 timeout 的 `Sys.sleep()` 延长到 17 秒。这样仍验证结构化 timeout、jobId 和临时目录清理，但不把机器负载或冷包加载误判为 R 图回归。
- 这不是产品 timeout 放宽：服务端默认和最小保护仍保持原值，用户运行时配置未修改。
- 防复发：发布时先单独执行本 smoke，再执行 CPU/IO 密集的全量测试或构建；普通 R renderer、雷达专项和 Docker parity 必须同时通过。
